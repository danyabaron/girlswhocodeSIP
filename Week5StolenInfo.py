...
def login(username,password):
    if not username in loginDict:
        ...
        password = raw_input("Please enter a new password to sign up: ")
...
def makeRainbowTable():
	string = "abcdeghijklmnopqrstuvwxyz"
	alphabet = list(string.upper())
	rainbowTable = {}
	for letterone in alphabet:
		for lettertwo in alphabet:
			for letterthree in alphabet:
				password = letterone+lettertwo+letterthree
				print password 
				rainbowTable[makeHash(password)]=password
	return rainbowTable

rainbowTable = makeRainbowTable()

Gerald = loginDict['Gerald'] #Gerald's hash
print rainbowTable[Gerald]
login("Gerald",rainbowTable[Gerald])
